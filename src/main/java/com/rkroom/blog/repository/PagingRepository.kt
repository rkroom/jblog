...
interface PagingRepository : JpaRepository<Article?, Int?> {
    ...
    fun findAllByPage(pageable: Pageable?): List<List<*>?>?
    ...
    fun findAllArticle(pageable: Pageable?): List<List<*>?>?
    ...
    fun findArticleByCategoryAndPage(category: String?, pageable: Pageable?): List<List<*>?>? //分类目录和分页数据作为参数
    ...
    fun findPreviousArticleSlug(id: Int, category: String?, pageable: Pageable?): List<List<*>?>?

    //查找下一篇文章，如果有分类参数，则查询该分类的下一篇文章
    @Query(value = "select a.slug,a.title from Article a where a.id > ?1 and a.published = true and a.categories.category like ?2 ")
    fun findNextArticleSlug(id: Int, category: String?, pageable: Pageable?): List<List<*>?>?
