...
interface PagingRepository : JpaRepository<Article?, Int?> {
    ...
    @Query(value = "select a.id,a.title,u.username,SUBSTRING(a.content,1,500),a.slug,a.publishdate,c.category from Article a left join User u on (a.users.id=u.id) left join Categories c on (a.categories.id = c.id) where a.published = true")
    ...
    //根据分类查询已经发表的文章
    //JPQL查询相应分类下已发表的文章
    @Query(value = "select a.id,a.title,SUBSTRING(a.content,1,500),a.slug,a.publishdate,u.username,c.category from Article a left join User u on (a.users.id=u.id) left join Categories c on (a.categories = c.id) where a.categories.category = ?1 and a.published = true ")
